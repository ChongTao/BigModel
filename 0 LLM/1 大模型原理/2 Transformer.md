...
Transformer 并不是停留在 2017 年原始结构不变的。后续很多能力增强，都是在这个家族上继续扩展出来的。

常见方向包括：

- **稀疏注意力 / 局部注意力**：减少超长序列时的计算压力，更适合长上下文场景。
- **RoPE 等位置编码扩展**：让模型在更长距离范围内仍然能较稳定地利用顺序信息。
- **MoE（混合专家）**：让不同专家模块处理不同输入模式，用更大参数规模换取更高计算效率。
- **多模态融合**：把文本、图像、语音等不同模态统一接入 Transformer 体系。

这些方向并没有改变 Transformer 的核心思想，而是在扩展它的输入类型、可训练规模和应用边界。

## 1.11 一句话总结

如果说：

- Token 解决“文本如何切成模型可处理的单位”
- 向量化解决“离散符号如何变成可计算对象”

那么：

- Attention 解决“当前 token 该重点参考谁”
- Transformer 解决“如何把这些上下文计算组织成深层神经网络”
